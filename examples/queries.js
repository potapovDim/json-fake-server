...
async function callToServer() {
  ...
  const query_resp_obj = await fetch('http://localhost:8081/test?testOne=1&testTwo=2', { method: 'GET' }).then(res =>
    res.text(),
  );
  // {"testOne":1,"testTwo":2}
  console.log(query_resp_obj);

  const query_resp_array = await fetch('http://localhost:8082/test?testOne=1&testTwo=2', { method: 'GET' }).then(res =>
    res.text(),
  );
