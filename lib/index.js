...
const formAction = (method, path, response, rest) => {
  const {
    ...
    errorResponse = { error: 'api.notfound' },
    requestBody = {},
    assertQuery = false,
  ...
  return {
    ...
    callCount: 0,
    assertQuery,
    queryAndBodyResponse: queryAndBodyResponse ? queryAndBodyResponse : response,
    method: method,
...
const formResponse = (
  ...
  if (responseFormat == 'text') {
    ...
    if (currentAction) {
      ...
      } else if (currentAction.assertQuery && !currentAction.assertQueryAndBody && !currentAction.assertRequest) {
        if (currentAction.requestQuery) {
          const equalQueries = currentAction.requestQuery === query;
          if (equalQueries) {
      ...
      } else if (currentAction.assertRequest && !currentAction.assertQuery) {
        const equal = deepEqual(requestBody, currentAction.requestBody);
        if (equal) {
          response.writeHead(successStatus, { 'Content-Type': 'application/json' });
          response.write(stringifyJson(currentAction.response))
      ...
      } else if (currentAction.assertQuery && currentAction.assertRequest) {
        const equalBodies = deepEqual(requestBody, currentAction.requestBody);
        const equalQueries = currentAction.requestQuery === query;
        if (equalBodies || equalQueries) {
          response.writeHead(successStatus, { 'Content-Type': 'application/json' });
...
class FakeServer {
  ...
  put({ path, response, ...rest }) {
    this.serverActions.push(formAction('PUT', path.includes('/') ? path : `/${path}`, response, rest))
  };
  post({ path, response, ...rest }) {
    this.serverActions.push(formAction('POST', path.includes('/') ? path : `/${path}`, response, rest))
  };
  get({ path, response, ...rest }) {
    this.serverActions.push(formAction('GET', path.includes('/') ? path : `/${path}`, response, rest))
  };
  del({ path, response, ...rest }) {
    this.serverActions.push(formAction('DELETE', path.includes('/') ? path : `/${path}`, response, rest))
  };
  start() {
    this.runned = true;
