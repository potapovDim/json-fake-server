...
/**
  ...
  * @returns {string}
*/
function post_template({path, response = {ok: 'OK'}, status = 200, params_response, authorization, response_from_url}) {
  ...
  if(response_from_url) {
    internal_body_part += build_request_from_other_server_response(response_from_url, path, 'POST')
  }

  if(params_response) {
    internal_body_part += build_request_params_response(params_response, path, 'POST')
  }

  internal_body_part += build_request_queries_response(response, path, 'POST')
